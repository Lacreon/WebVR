(function () {
    ...
    VRManager.prototype.animate = function () {
        ...
        document.dispatchEvent(new CustomEvent('VRData', {
            detail: {
    ...
    VRManager.prototype.unityMessage = function (msg) {
        ...
        if (msg.detail.type === 'displayElementId') {
          ...
          if (msg.detail.type === 'setVRModeEnabled') {
            if(msg.detail.enabled === 'false'){
                if (this.vrDisplay && this.vrDisplay.isPresenting && this.gameInstance) {
                    this.exitPresent();
                }
            } else {
    ...
    function getConfigFilePath() {
        var path = getUrlParam('configurationFilePath', 'null');
        gameInstance.SendMessage('PanoramaTourWrapper', 'OnConfiguartionFilePath', path);
    }
